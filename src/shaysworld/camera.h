...
public:
	...
	void CheckCamera();
	 /**
	*@brief plays footstep sounds
	*this function checks to see how much time has passed
	*if enough time has passed a sound will be played and 
	*the sound limit time will be increased.
	*
	*@return void
	*/
	void Footsteps();
	...
	/**
	*@brief changes to game world
	*
	*this function checks the location of the player
	*if they are standing directly infront of the painting 
	*the program will switch to gameworld by calling the 
	*function GameLoop(); from the gamemain class
	*
	*@return void
	*
	*/
