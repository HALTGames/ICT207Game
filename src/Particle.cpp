...
void Particle::CreateMissile(int i, Vector3 Start, Vector3 Direction)
{
	 ...
	 MissileEffect[i].xpos= Start.x;
     MissileEffect[i].ypos= Start.y;
     MissileEffect[i].zpos= Start.z;
     MissileEffect[i].xspeed = (-Direction.x)-(float)(rand()%100)/100000.0;
     MissileEffect[i].yspeed = (-Direction.y)-(float)(rand()%100)/100000.0;
     MissileEffect[i].zspeed = (-Direction.z)-(float)(rand()%100)/100000.0;
...
void Particle::DrawMissile(Vector3 Start, Vector3 Direction)
{
	glLoadIdentity();
	...
	for (int i=0;i<=MaxParticle;i++)
	{
		...
		{
			CreateMissile(i, Start, Direction);
		}
    }
	DecayMissile();
}